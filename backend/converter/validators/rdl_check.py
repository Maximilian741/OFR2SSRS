...
def _check_parameter_consistency(root: ET.Element) -> List[Dict[str, Any]]:
    """Every <QueryParameter Name='@P_X' or ':P_X'> must have a matching
    <ReportParameter Name='P_X'>.

    Oracle data sources emit QueryParameter names with a leading ':' (bind
    variable syntax); SQL Server uses '@'. Strip both so the consistency
    check works for either target. The match is case-insensitive because
    the generator does a case-insensitive canonical lookup when binding
    QueryParameter values to ReportParameters - so a QueryParameter named
    ':Prog_Id' is correctly bound to a ReportParameter named 'PROG_ID'.
    """
    issues: List[Dict[str, Any]] = []

    # Collect declared report parameters (upper-cased for case-insensitive
    # match; the original casing is what the generator's Value expression
    # uses, so emitting it doesn't matter for validity).
    declared: set = set()
    for rp in _findall_local(root, "ReportParameter"):
        nm = rp.get("Name")
        if nm:
            declared.add(nm.lstrip("@:").upper())

    seen_query_params: Dict[str, str] = {}  # upper-bare name -> original bare
    for qp in _findall_local(root, "QueryParameter"):
        raw = qp.get("Name") or ""
        bare = raw.lstrip("@:")
        if not bare:
            continue
        seen_query_params.setdefault(bare.upper(), bare)

    for upper_bare, bare in sorted(seen_query_params.items()):
        if upper_bare not in declared:
            issues.append(_make(
    ...
    # Build a {upper -> original} map so the info-level message uses the
    # original casing the report author wrote.
    declared_pairs: Dict[str, str] = {}
    for rp in _findall_local(root, "ReportParameter"):
        nm = rp.get("Name")
        if nm:
            bare = nm.lstrip("@:")
            declared_pairs.setdefault(bare.upper(), bare)
    used = set(seen_query_params.keys())
    for upper, original in sorted(declared_pairs.items()):
        if upper in used:
            continue
        issues.append(_make(
            "info", "rdl.parameter_consistency",
            f"<ReportParameter> '{original}' is declared but never referenced by "
            "any <QueryParameter>; the prompt will appear with no effect.",
            f"ReportParameter[{original}]",
        ))
    return issues
...
def validate_rdl(rdl_xml: str, target_db: str = "sqlserver") -> List[Dict[str, Any]]:
    """Run every static check against the RDL XML and return a flat issue list.

    ``target_db`` controls dialect-specific checks. When the target is
    ``"oracle"`` the Oracle-leftovers check is skipped, since DECODE/NVL/
    SYSDATE/etc. are legitimate against an Oracle data source.
    """

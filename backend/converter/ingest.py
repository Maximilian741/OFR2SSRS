...
def convert_bundle(files: List[Tuple[str, bytes]], target_db: str = "oracle") -> Dict[str, Any]:
    ...
    if primary_xml:
        ...
        from .artifact_enrich import enrich_report_from_artifacts
        try:
        ...
        # Parent-report artifact stacking: only run when at least one
        # supporting artifact came along with the XML. When the bundle is
        # XML-only the original convert() output is preserved byte-for-byte
        # (no enrichment, no re-generation), so XML-only bundles stay
        # regression-free against plain convert().
        has_extras = bool(
            (classification.get("sql_files") or [])
            or (classification.get("docs") or [])
            or (classification.get("screenshots") or [])
        )
        if has_extras:
            try:
                parsed_enrich = parse_oracle_xml(primary_xml[1])
                applied = enrich_report_from_artifacts(parsed_enrich, classification)
                # Only re-run translation + RDL generation if something
                # actually changed; otherwise leave the original outputs alone.
                changed = bool(applied.get("sql_added") or applied.get("sql_replaced")
                               or applied.get("label_overrides"))
                if changed:
                    try:
                        translate_report(parsed_enrich)
                    except Exception as _te:
                        parsed_enrich.warnings.append(f"Translator error: {_te}")
                    try:
                        new_rdl = generate_rdl(parsed_enrich, target_db=target_db)
                        data["rdl_xml"] = new_rdl
                        data["report"] = parsed_enrich.to_dict()
                    except Exception as _re:
                        parsed_enrich.warnings.append(f"RDL regeneration error: {_re}")
                data["artifacts_enriched"] = applied
            except Exception as _ee:
                data["artifacts_enriched"] = {
                    "sql_added": 0, "sql_replaced": 0,
                    "label_overrides": 0, "hints": [],
                    "error": f"{type(_ee).__name__}: {_ee}",
                }
    ...
    # Path 2: no XML, but we have SQL or docs containing SQL -- synthesize.
    docs = classification.get("docs") or []
    screenshots = classification.get("screenshots") or []
    if sql_files:
        from .artifact_enrich import enrich_synthetic_from_artifacts
        label = "BUNDLE"
        ...
        for fn, _ in sql_files:
            ...
            if stem:
                ...
                break
        report, applied = enrich_synthetic_from_artifacts(
            sql_files, docs, screenshots, bundle_label=label
        )
        try:
        ...
        return {
            ...
            "rdl_xml": rdl_xml,
            "oracle_xml": "",
            "mockup_html": "<em>No Oracle layout available -- synthetic report from artifacts.</em>",
            "ingest_report": ingest,
    ...
    # hand the user the exact escape hatch: Oracle's own rwconverter ships
    # with every Reports/Developer install and batch-exports .rdf -> XML,
    # which THIS tool then converts.
    out = {
        "error": "no_convertible_artifacts",
        "ingest_report": ingest,
    }
    rdf = classification.get("rdf_binary")
    if rdf:
        from pathlib import Path as _P
        out["rdf_hint"] = (
            "'{name}' is an Oracle Reports BINARY (.rdf), which only "
            "Oracle's tools can read. One-time export on any machine with "
            "Oracle Reports/Developer installed:\n\n"
            "  rwconverter userid=USER/PASS@DB batch=yes source={name} "
            "stype=rdffile dtype=xmlfile dest={base}.xml\n\n"
            "(for a whole folder: source=*.rdf) -- then drop the .xml here."
        ).format(name=rdf[0], base=_P(rdf[0]).stem)
    return out
